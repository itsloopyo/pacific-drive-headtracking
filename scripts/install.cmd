...
:: alone. Contract: see the header of that template.
::
...
:: Deployed only when ABSENT, so a reinstall or an update keeps whatever the
:: user configured. Listing it in MOD_DLLS put it through the unconditional
:: copy /y below, which silently reset FovOffset, smoothing, lean limits and
:: every keybind on every update - deploy.ps1 already guarded the same copy.
...
set "MOD_CONTROLS=Toggle: End or Ctrl+Shift+Y  ^|  Cycle tracking mode: PageUp or Ctrl+Shift+G  ^|  Toggle yaw mode: PageDown or Ctrl+Shift+H"
...
:: See vendor/ultimate-asi-loader/README.md for the vendored snapshot.
:: ============================================
:install_asi_loader
