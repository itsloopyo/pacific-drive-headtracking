...
// The build registry. Profiles live one file per store (steam_offsets.cpp,
// gdk_offsets.cpp) and are listed newest first here; the top entry is the
// diagnostic primary the "unknown build" log line compares against. Adding a
// build means adding an entry, never editing one - a player still on the old
// build keeps matching the profile they have.
const BuildProfile* const kKnownProfiles[] = {
    &kGdkProfile_20260911,
